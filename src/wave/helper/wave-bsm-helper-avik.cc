...
NS_LOG_COMPONENT_DEFINE ("WaveBsmHelperAvik");

namespace ns3 {

std::vector<int> WaveBsmHelper::nodesMoving;

WaveBsmHelperAvik::WaveBsmHelperAvik ()
  : m_waveBsmStats ()
{
...
void
WaveBsmHelperAvik::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer
WaveBsmHelperAvik::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer
WaveBsmHelperAvik::Install (Ipv4InterfaceContainer i) const
{
...
Ptr<Application>
WaveBsmHelperAvik::InstallPriv (Ptr<Node> node) const
{
...
void
WaveBsmHelperAvik::Install (Ipv4InterfaceContainer & i,
                        ...
                        Time txMaxDelay)         // max delay prior to transmit
{
  ...
  for (aci = bsmApps.Begin (); aci != bsmApps.End (); ++aci)
    ...
    std::cout << "finished setup of nodes" << '\n';
}


Ptr<WaveBsmStats>
WaveBsmHelperAvik::GetWaveBsmStats ()
{
  return &m_waveBsmStats;
}
Ptr<WaveBsmStats>
WaveBsmHelperAvik::GetWaveBsmStats_hp ()
{
  return &m_waveBsmStats_hp;
}
int64_t
WaveBsmHelperAvik::AssignStreams (NodeContainer c, int64_t stream)
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    ...
    std::cout << "finished assigning streams" << '\n';
  return (currentStream - stream);
}

std::vector<int>&
WaveBsmHelperAvik::GetNodesMoving ()
{
  return nodesMoving;
}
void printAppStats(std::string CSVfileName3);
void printStats(std::string CSVfileName3){
  printAppStats(CSVfileName3);
}
} // namespace ns3
