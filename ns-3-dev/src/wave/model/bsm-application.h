...
private:
   ...
   * \return none
   */
   void GeneratePriorityWaveTraffic (Ptr<Socket> socket, uint32_t pktSize,
                            uint32_t pktCount, Time pktInterval,
                            uint32_t sendingNodeId);
  /**
  ...
  void ReceiveAdaptiveWavePacket (Ptr<Socket> socket);
  /**
  ...
  Ptr<WaveBsmStats> m_waveBsmStats_hp; ///< BSM stats
