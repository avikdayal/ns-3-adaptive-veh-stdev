  ...
  for (uint32_t i = 0; i < nModes; i++)
    {
      ...
      std::cout << ""<< i << " " << time.GetSeconds () << " " << dataRate<<std::endl;
...
NodeStatistics::PowerCallback (std::string path, double oldPower, double newPower, Mac48Address dest)
{
  ...
  std::cout << "old power" << oldPower<< '\n';
  std::cout << "new power" << newPower<< '\n';
}
...
int main (int argc, char *argv[])
{
  ...
  std::cout << "Setting initial AP position to " << Vector (ap1_x, ap1_y, 0.0) << std::endl;
  positionAlloc->Add (Vector (sta1_x, sta1_y, 0.0));
  NS_LOG_INFO ("Setting initial STA position to " << Vector (sta1_x, sta1_y, 0.0));
  std::cout << "Setting initial STA position to " << Vector (sta1_x, sta1_y, 0.0) << std::endl;
  ...
  std::ofstream outfile (("throughput2-" + outputFileName + ".plt").c_str ());
  Gnuplot gnuplot = Gnuplot (("throughput2-" + outputFileName + ".eps").c_str (), "Throughput");
  ...
  if (manager.compare ("ns3::ParfWifiManager") == 0
    ...
    {
      std::ofstream outfile2 (("power2-" + outputFileName + ".plt").c_str ());
      gnuplot = Gnuplot (("power2-" + outputFileName + ".eps").c_str (), "Average Transmit Power");
      gnuplot.SetTerminal ("post eps color enhanced");
