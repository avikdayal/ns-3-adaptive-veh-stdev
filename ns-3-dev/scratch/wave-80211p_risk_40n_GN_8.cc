 ...
 */
 ...
 struct NodePos {
 	...
 	NodePos() :
 			 ...
 			 m_y(0),
 			 m_z(0) {}
 	NodePos(double x, double y, double z,double vx, double vy, double vz) :
 			 m_send_rec_flag(' '),
 			 m_x(x),
 			 m_y(y),
 			 m_z(z),
       m_vx(vx),
 			 m_vy(vy),
 			 m_vz(vz) {}
 	NodePos(char send_rec, double x, double y, double z,double vx, double vy, double vz) :
 ...
 struct NodeState {
 	...
 	void SendPacket(uint32_t m_Node_Id, uint32_t m_Msg_Id,
 			double m_x, double m_y, double m_z,double m_vx, double m_vy, double m_vz)
 	{
 		NodePos ns('S', m_x, m_y, m_z, m_vx, m_vy, m_vz);
 		stats[m_Node_Id] = ns;
 	...
 	void RecvPacket(uint32_t m_Node_Id, uint32_t m_Msg_Id,
 			double m_x, double m_y, double m_z,double m_vx, double m_vy, double m_vz)
 	{
 		NodePos ns('R', m_x, m_y, m_z, m_vx, m_vy, m_vz);
 		stats[m_Node_Id] = ns;
 	}
 	void SetNodeLocation(uint32_t m_Node_Id,
 			double m_x, double m_y, double m_z,double m_vx, double m_vy, double m_vz)
 	{
 		NodePos ns(' ', m_x, m_y, m_z, m_vx, m_vy, m_vz);
 		stats[m_Node_Id] = ns;
 	}
...
void CourseChange (std::ostream *os, std::string context, Ptr<const MobilityModel> mobility)
{
	  ...
	  Current_pos.SetNodeLocation(nodeId,pos.x, pos.y, pos.z, vel.x, vel.y, vel.z);
...
int main (int argc, char *argv[])
{
  ...
  double hp_interval = 0.05; // seconds
  bool verbose = false;
