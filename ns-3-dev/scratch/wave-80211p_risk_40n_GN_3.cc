                             ...
                             uint32_t pktCount, Time pktInterval )
{
  ...
  //nodes_in_range=Current_pos.NodesInRange(socket->GetNode()->GetId(),100);
	...
	std::cout << Simulator::Now().GetMilliSeconds() << ",S,"
			<< socket->GetNode()->GetId() << "," << tag.GetSimpleValue()<< "," << priority;
	//Current_pos.PrintDistFrom(socket->GetNode()->GetId());
  Current_pos.PrintNodesInRange(socket->GetNode()->GetId(),g_safetyDistSq);
	std::cout << std::endl;
	socket->Send(pkt);
...
int main (int argc, char *argv[])
{
  ...
  */
  ObjectFactory pos;
    ...
    speed_threshold=4*m_nodeSpeed/5;
  ...
  for (int i = 0; i < nSinks; i++)
    {
      ...
      recvSink->SetRecvCallback (MakeCallback (&ReceivePacket));
        ...
        NodeIdToDeviceMap[nodes.Get(i)->GetId()] = phy;
