  ...
  while (socket->RecvFrom (from))
    {
      ...
      std::cout << "@" << Simulator::Now ().GetSeconds () << " Received one packet! Node Id=" <<socket->GetNode()->GetId() <<", sender = "<<from<<  std::endl;
    }
    /*
    while (socket->Recv ())
      {
